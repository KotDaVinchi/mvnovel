...
Формат: `%перс (индентификаторБезПробелов) (ИмяБезПробелов) {(json data)} (default)`
```
%перс musician Джон
%перс mom Мама {{"defaultExt":"webp"}} default
%перс
```
...
нельзя делать короче 3х символов.
...
"add", "sub", "mul", "div" для математических действий
```
%действие add counter 5
%действие mul counter 2
```

"saveWardrobe", "restoreWardrobe" для сохранения и восстановления состояния гардероба
```
%действие saveWardrobe
%действие restoreWardrobe
```
...
В тексте сюжета так-же можно передавать дополнительную информацию при помощи команд "сцена" и "допдата".

Команда "сцена" позволяет дописывать различные технические данные в текущее состояние, например:
```
Конец
%конец
%сцена {"endingMessage": "Поздравляем! Это $endingsCount из $endingsAmount концовок."}
```
Команда "допдата" позволяет дописывать данные в определение истории
```
%допдата {"knownCharsByDefault": true}
%допдата {"disableWardrobe": true}
%допдата {"endingCredits": "Над историей работали:\nМного прекрасных людей."}
```
